...
- Text clip thumbnails display a live mini-preview of the actual clipboard content
- Image clip thumbnails show a hover preview up to 680×560 points, auto-sized to the image's aspect ratio
...
- Pinned items appear in a dedicated **Pinned** section above the **Recent** list; the footer shows a pause indicator when recording is paused
...
| Preview an image at full size | Hover over its thumbnail |
...
4. [`ClipboardRepository.swift`](Sources/Maclipp/Storage/ClipboardRepository.swift) deduplicates clips by content hash, keeps at most 100 records, and atomically persists [`ClipboardItem`](Sources/Maclipp/Clipboard/ClipboardItem.swift) metadata as JSON. Images are normalized to separate PNG files, and all persisted data uses owner-only permissions. Loaded images are held in an in-memory `NSCache` so list renders do not repeatedly hit the file system; the cache entry is evicted when the item is deleted.
5. [`MenuBarController.swift`](Sources/Maclipp/UI/MenuBarController.swift) anchors the popover to the menu-bar icon on the active display. [`ClipboardPanelView.swift`](Sources/Maclipp/UI/ClipboardPanelView.swift) provides search, text-content thumbnails, image hover previews, newest-item selection, and `Command+1` through `Command+5` access to the five most recent matching items. When pinned items are present the list is divided into **Pinned** and **Recent** sections.
